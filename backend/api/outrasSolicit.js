module.exports = (app) => {
    ...
    const getOutras = async (req, res) => {
      try {
        ...
        const trocas = await app
          ...
          const dataFormatada = troca.dt_processo.split('-').reverse().join('/');
        
                return { ...troca, dt_processo: dataFormatada };
              });
    ...
    const getById = async (req, res) => {
      const responsavel = await app
        .db("outras_solicitacoes")
        .join("users", "outras_solicitacoes.id_administrativo", "=", "users.id")
        .select("users.nome_funcional")
        .where("outras_solicitacoes.id","=",req.params.id)
        .first();

      app
      .db("outras_solicitacoes")
        .select()
        .table("outras_solicitacoes")
        ...
        .then((troca) => {
          ...
          const formatedDtocorrencia = troca.dt_processo.split("-").reverse().join("/");
          
          res.json({ ...troca, dth_sist: formatedDtsistema, dt_processo: formatedDtocorrencia, responsavel: responsavel, });
        })
    ...
    return { getOutras, getById, updateState };
  };
