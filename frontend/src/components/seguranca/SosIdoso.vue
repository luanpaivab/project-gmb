<template>
  <div class="sos-idoso">
    <PageTitle icon="fa fa-user" main=" SOS Idoso" sub="" />
    <hr />
    <b-col md="4">
      <div class="stats">
        <Stat
    ...
    <div id="content-tabela-idoso">
      ...
      >
        <template slot="links" slot-scope="data">
          <router-link
            :to="{ name: 'sosIdosoId', params: { id: data.item.id } }"
          >
            Visualizar
          </router-link>
        </template>
...
export default {
  ...
  methods: {
    loadDenuncias() {
      const url = `${baseApiUrl153}/sosidoso?page=${this.page}&registro=${this.registro}&tipo=${this.tipo}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}`;
      axios(url).then((res) => {
    ...
    CriaPDF() {
      var minhaTabela = document.getElementById("content-tabela-idoso")
        .innerHTML;
      var style = "<style>";
      style =
        style +
