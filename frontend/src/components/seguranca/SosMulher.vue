<template>
  <div class="sos-mulher">
    <PageTitle icon="fa fa-user" main=" SOS Mulher" sub="" />
    <hr />
    <b-col md="4">
      <div class="stats">
        <Stat
  ...
  <div id="content-tabela-mulher">
    ...
    >
      <template slot="links" slot-scope="data">
        <router-link
          :to="{ name: 'sosMulherId', params: { id: data.item.id } }"
        >
          Visualizar
        </router-link>
      </template>
...
export default {
  ...
  methods: {
    loadDenuncias() {
      const url = `${baseApiUrl153}/sosmulher?page=${this.page}&registro=${this.registro}&tipo=${this.tipo}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}`;
      axios(url).then((res) => {
    ...
    CriaPDF() {
      var minhaTabela = document.getElementById("content-tabela-mulher")
        .innerHTML;
      var style = "<style>";
      style =
        style +
