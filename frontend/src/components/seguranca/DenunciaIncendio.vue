<template>
  <div class="denuncia-incendio">
    <PageTitle icon="fa fa-home" main=" Denuncia de Incêndios" sub="" />
    <hr />    
    <b-col md="4">
      <div class="stats">
        <Stat
          title="Denuncia Incêndios"
          :value="count"
          icon="fa fa-fire"
          color="red"
        />
  ...
  <div id="content-tabela-incendio">
    <b-table
      class="mt-4"
      hover
      :items="denuncias"
    ...
    >
      <template slot="links" slot-scope="data">
        <router-link
          :to="{ name: 'denunciaIncendioId', params: { id: data.item.id } }"
        >
          Visualizar
        </router-link>
      </template>
...
export default {
  ...
  data: function () {
    return {
      ...
      fields: [
        ...
        { key: "hr_ocorrencia", label: "Hora da Ocorrência", sortable: true },
        { key: "local_incendio", label: "Local do Incêndio", sortable: true },
  ...
  methods: {
   /* loadDenuncias() {
      const url = `${baseApiUrl153}/denunciaincendio?page=${this.page}`;
      axios.get(url).then((res) => {
        this.denuncias = res.data.data;
        this.count = res.data.count;
        this.limit = res.data.limitStats;
      });
    },*/
    loadDenuncias() {
      const url = `${baseApiUrl153}/denunciaincendio?page=${this.page}&registro=${this.registro}&local=${this.local}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}&hora=${this.hora}`;
      axios(url).then((res) => {
        this.denuncias = res.data.data;
        this.count = res.data.count;
        this.limit = res.data.limitStats;
    ...
    CriaPDF() {
      var minhaTabela = document.getElementById("content-tabela-incendio")
        .innerHTML;
      var style = "<style>";
      style =
        style +
