<template>
  <div class="home">
    <PageTitle icon="fa fa-car" main=" Veiculos Roubados" sub="" />
    <hr />
    <b-col md="4">
      <div class="stats">
        <Stat
          title="Veiculos roubados"
          :value="count"
          icon="fa fa-car"
          color="#3bc480"
        />
    ...
    <b-row>
      ...
      <b-col md="3">
        <b-form-group label="Veiculo:" label-for="veiculo-veiculo" class="pl-3">
          <b-form-input
            id="filter-veiculo"
            v-model="veiculo"
            type="search"
    ...
    <div id="content-tabela-veiculos">
      <b-table
        class="mt-4"
        hover
        :items="veiculos"
      ...
      >
        <template slot="links" slot-scope="data">
          <router-link
            :to="{ name: 'veiculosRoubadosId', params: { id: data.item.id } }"
          >
            Visualizar
          </router-link>
        </template>
...
export default {
  ...
  methods: {
    loadVeiculosFiltered() {
      const url = `${baseApiUrl153}/veiculosroubados?page=${this.page}&registro=${this.registro}&veiculo=${this.veiculo}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}&hora=${this.hora}`;
      axios(url).then((res) => {
        this.veiculos = res.data.data;
    ...
    clear() {
      ...
      this.hora = "";
      this.veiculo = "";
      this.loadVeiculosFiltered();
    },
    CriaPDF() {
      var minhaTabela = document.getElementById("content-tabela-veiculos")
        .innerHTML;
      var style = "<style>";
      style =
        style +
