<template>
  <div class="home">
    <PageTitle icon="fa fa-user" main=" Roubos e Furtos" sub="" />
    <hr />
    <b-col md="4">
      <div class="stats">
        <Stat
  ...
  <div id="content-tabela-roubos">
    ...
    >
      <template slot="links" slot-scope="data">
        <router-link
          :to="{ name: 'roubosFurtosId', params: { id: data.item.id } }"
        >
          Visualizar
        </router-link>
      </template>
...
export default {
  ...
  methods: {
    /*loadRoubos() {
      const url = `${baseApiUrl153}/roubosefurtos?page=${this.page}`;
    ...
    loadRoubos() {
      const url = `${baseApiUrl153}/roubosefurtos?page=${this.page}&registro=${this.registro}&arma=${this.arma}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}&hora=${this.hora}`;
      axios(url).then((res) => {
        this.roubos = res.data.data;
        this.count = res.data.count;
        this.limit = res.data.limitStats;
    ...
    CriaPDF() {
      var minhaTabela = document.getElementById("content-tabela-roubos")
        .innerHTML;
      var style = "<style>";
      style =
        style +
