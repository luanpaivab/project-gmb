<template>
  <div class="home">
    <PageTitle icon="fa fa-car" main=" Acidentes de Trânsito" sub="" />
    <hr />
    <b-col md="4">
      <div class="stats">
        <Stat
  ...
  <div id="content-tabela-acidentes">
    <b-table
      class="mt-4"
      hover
      :items="acidentes"
    ...
    >
      <template slot="links" slot-scope="data">
        <router-link
          :to="{ name: 'acidenteDeTransitoId', params: { id: data.item.id } }"
        >
          Visualizar
        </router-link>
      </template>
...
export default {
  ...
  data: function () {
    return {
      ...
      fields: [
        ...
        { key: "tipo_acidente", label: "Tipo de Acidente", sortable: true },
  ...
  methods: {
    loadAcidentes() {
      const url = `${baseApiUrl153}/acidentetransito?page=${this.page}&registro=${this.registro}&tipo=${this.tipo}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}`;
      axios(url).then((res) => {
        this.acidentes = res.data.data;
    ...
    CriaPDF() {
      var minhaTabela = document.getElementById("content-tabela-acidentes")
        .innerHTML;
      var style = "<style>";
      style =
        style +
