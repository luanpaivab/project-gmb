<template>
  <div class="sos-menores">
    <PageTitle icon="fa fa-user" main=" SOS Criança e Adolescente" sub="" />
    <hr />
    <b-col md="5">
      <div class="stats">
        <Stat
  ...
  <div id="content-tabela-menores">
    ...
    >
      <template slot="links" slot-scope="data">
        <router-link
          :to="{ name: 'sosMenoresId', params: { id: data.item.id } }"
        >
          Visualizar
        </router-link>
      </template>
...
export default {
  ...
  data: function () {
    return {
      ...
      fields: [
        ...
        { key: "tipo_agressao", label: "Tipo de Agressão", sortable: true },
  ...
  methods: {
    loadDenuncias() {
      const url = `${baseApiUrl153}/sosmenores?page=${this.page}&registro=${this.registro}&tipo=${this.tipo}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}`;
      axios(url).then((res) => {
    ...
    CriaPDF() {
      var minhaTabela = document.getElementById("content-tabela-menores")
        .innerHTML;
      var style = "<style>";
      style =
        style +
