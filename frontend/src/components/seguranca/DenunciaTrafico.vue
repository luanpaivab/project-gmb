<template>
  <div class="denuncia-trafico">
    <PageTitle icon="fa fa-home" main=" Denuncia de Tráfico" sub="" />
    <hr />
       <b-col md="4">
      <div class="stats">
        <Stat
          title="Denuncia de Tráfico"
          :value="count"
          icon="fa fa-flask"
          color="#000"
        />
  ...
  <div id="content-tabela-trafico">
    ...
    >
      <template slot="links" slot-scope="data">
        <router-link
          :to="{ name: 'denunciaTraficoId', params: { id: data.item.id } }"
        >
          Visualizar
        </router-link>
      </template>
...
export default {
  ...
  methods: {
    loadDenuncias() {
      const url = `${baseApiUrl153}/denunciatrafico?page=${this.page}&registro=${this.registro}&tipo=${this.tipo}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}`;
      axios(url).then((res) => {
    ...
    CriaPDF() {
      var minhaTabela = document.getElementById("content-tabela-trafico")
        .innerHTML;
      var style = "<style>";
      style =
        style +
