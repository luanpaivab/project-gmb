<template>
  <div class="home">
    <PageTitle icon="fa fa-trash" main=" Descartes Irregulares de Lixo" sub="" />
    <hr />
    <b-col md="4">
      <div class="stats">
        <Stat
  ...
  <div id="content-tabela-descarte">
    <b-table
      class="mt-4"
      hover
      :items="descartes"
    ...
    >
      <template slot="links" slot-scope="data">
        <router-link :to="{ name: 'descarteIrregularId', params: { id: data.item.id } }">
          Visualizar
        </router-link>
      </template>
    </b-table>
...
export default {
  ...
  data: function () {
    return {
      ...
      fields: [
        ...
        { key: "tipo_descarte_lixo", label: "Tipo de Descarte", sortable: true },
  ...
  methods: {
    loadDescartes() {
      const url = `${baseApiUrl153}/descarteirregular?page=${this.page}&registro=${this.registro}&tipo=${this.tipo}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}`;
      axios(url).then((res) => {
        this.descartes = res.data.data;
    ...
    CriaPDF() {
      var minhaTabela = document.getElementById("content-tabela-descarte")
        .innerHTML;
      var style = "<style>";
      style =
        style +
