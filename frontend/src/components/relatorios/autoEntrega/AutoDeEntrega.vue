<template>
  <div class="auto-entrega">
    <PageTitle icon="fa fa-home" main=" Auto de Entrega" />
    <hr />
    <b-col md="4">
      <div class="stats">
        <Stat
          title="Autos de Entrega"
          :value="count"
          icon="fa fa-file"
          color="#000"
        />
    ...
    <b-row>
      ...
      <b-col md="2">
        <b-form-group label="Ro Vinculado:" label-for="ro-ro">
          <b-form-input
            id="filter-ro"
            v-model="ro"
            type="search"
    ...
    >
      <template slot="links" slot-scope="data">
        <router-link
          :to="{ name: 'autoDeEntregaId', params: { id: data.item.id } }"
        >
          <i
            :class="`fa fa-file 
              ${(data.item.state === 'OK') 
...
export default {
  ...
  data: function () {
    return {
      ...
      fields: [
        ...
        { key: "natureza_ocorrencia", label: "Natureza", sortable: true },
  ...
  methods: {
    loadAutos() {
      const url = `${baseApiUrl}/autoentrega?page=${this.page}&registro=${this.registro}&natureza=${this.natureza}&condutor=${this.condutor}&ro=${this.ro}&dataInicial=${this.dataInicial}&dataFinal=${this.dataFinal}`;
      axios(url).then((res) => {
