<template>
  <div>
    <div class="relatorio-disparo-id">
      <PageTitle
        icon="fa fa-file-o"
        text=" Relatório de Disparo - "
        :main="relatorio.id"
        sub=""
      />
      ...
      <div v-if="relatorio.state === 'ERROR' && erro_n_existe === true">
        <b-alert show variant="danger">
          ...
          <b-button
            ...
            variant="secondary"
            v-on:click="
              (relatorio.state = ''), (relatorio.state_description = '')
            "
            @click="() => Atualizando()"
          >
            Cancelar
      ...
      <div
        v-if="relatorio.state === 'ERROR' && erro_existe === true"
        class="alerta"
      >
        <b-alert show variant="danger">
      ...
      <div class="content" id="content-relatorio-disparo">
        <hr />
        <div class="titulo">
          <h2>Identificação Circunstanciado de Disparo de Arma de Fogo</h2>
        </div>
        <hr />
        <b-row>
          <b-col md="3">
        ...
        <b-row>
          <b-col md="4">
            <div class="test">
              <p>Hora do Disparo:</p>
              <div class="campo">
                {{ relatorio.hr_disparo }}
              </div>
            </div>
          </b-col>
          <b-col md="6">
            <div class="test">
              <p>Local do Disparo:</p>
              <div class="campo">
                {{ relatorio.perimetro }}
              </div>
            </div>
        ...
        <hr />
        <div class="titulo"><h2>Identificação do Servidor</h2></div>
        <hr />
        ...
        <hr />
        <div class="titulo"><h2>Dados do Armamento Utilizado</h2></div>
        <hr />
        ...
        <hr />
        <div class="titulo"><h2>Dados da Munição</h2></div>
        <hr />
        ...
        <b-row>
          <b-col md="8">
            <div class="test">
              <p>Complemento 1º Testemunha:</p>
              <div class="campo">
                {{ relatorio.complemento_endTestUm }}
              </div>
            </div>
          </b-col>
        </b-row>
        ...
        <b-row>
          <b-col md="8">
            <div class="test">
              <p>Complemento 2º Testemunha:</p>
              <div class="campo">
                {{ relatorio.complemento_endTestDois }}
              </div>
            </div>
          </b-col>
        </b-row>
        ...
        <hr />
        <div class="titulo"><h2>Relato da Ocorrência</h2></div>
        <hr />
    ...
    <b-col class="mb-2">
      <b-button
        ...
        variant="success"
        v-on:click="
          (relatorio.state = 'OK'), (relatorio.state_description = ''), (relatorio.id_administrativo = user.id)
        "
        @click="() => Atualizando()"
      >
...
export default {
  ...
  methods: {
    loadRelatorio() {
      const url = `${baseApiUrl}/relatoriodisparo/${this.$route.params.id}`;
    ...
    Atualizando() {
      ...
      axios
        .put(`${baseApiUrl}/relatoriodisparo/${id}`, this.relatorio)
        .then(() => {
          this.loadRelatorio();
        })
        .catch((error) => {
    ...
    CriaPDF() {
      var minhaTabela = document.getElementById("content-relatorio-disparo")
        .innerHTML;
      var style = "<style>";
      style =
        style +
