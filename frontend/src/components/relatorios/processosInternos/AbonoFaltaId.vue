<template>
  <div class="abono-falta-id">
    <div>
      ...
      <div v-if="abono.state === 'ERROR' && erro_n_existe === true">
        <b-alert show variant="danger">
          <h4 class="alert-heading">
            Documento com Erro! Verificado por -
            <strong> {{ abono.responsavel.nome_funcional }}</strong>
          </h4>
          <b-form-textarea
            id="textarea"
            placeholder="Relate o erro..."
            v-model="abono.state_description"
            rows="4"
          ></b-form-textarea>
          <b-button
            class="mt-2"
          ...
          <b-button
            class="mt-2"
            variant="secondary"
            v-on:click="(abono.state = ''), (abono.state_description = '')"
            @click="() => Atualizando()"
          >
            Cancelar
          </b-button>
      ...
      <div
        v-if="abono.state === 'ERROR' && erro_existe === true"
        class="alerta"
      >
        <b-alert show variant="danger">
          <h4 class="alert-heading">
      ...
      <div class="content" id="content-abono-falta">
        <hr />
        <div class="titulo">
          <h2>Processo Interno Para Abono de Falta</h2>
        </div>
        <hr />
        <b-row>
          <b-col md="3">
        ...
        <div class="titulo pt-4">
          <h2>Dados dos Dias para Abonar a Falta em Serviço</h2>
        </div>
        <hr />
    ...
    <b-col>
      <b-button
        ...
        v-on:click="
          (abono.state = 'OK'),
            (abono.state_description = ''),
            (abono.id_administrativo = user.id)
        "
        @click="() => Atualizando()"
      >
      ...
      <b-button
        ...
        variant="danger"
        v-on:click="
          (abono.state = 'ERROR'), (abono.id_administrativo = user.id)
        "
...
export default {
  ...
  methods: {
    CriaPDF() {
      var minhaTabela = document.getElementById("content-abono-falta")
        .innerHTML;
      var style = "<style>";
      style =
        style +
    ...
    loadAbono() {
      const url = `${baseApiUrl}/abonofalta/${this.$route.params.id}`;
    ...
    Atualizando() {
      const id = this.abono.id;

      axios
        .put(`${baseApiUrl}/abonofalta/${id}`, this.abono)
        .then(() => {
          this.loadAbono()
        })
        .catch((error) => {
