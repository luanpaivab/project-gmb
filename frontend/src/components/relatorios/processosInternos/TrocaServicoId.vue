<template>
  <div class="troca-servico-id">
    <div >
      <PageTitle
        icon="fa fa-file-o"
        text=" Troca de Serviço - "
        :main="troca.id"
        sub=""
      />
  ...
  <!-- Se eu estiver declarando um erro pela primeira vez, essa div aparece com uma caixa de texto para eu relatar o erro e add ao campo state description -->
      <div v-if="troca.state === 'ERROR' && erro_n_existe === true">
        <b-alert show variant="danger">
          <h4 class="alert-heading">
            Documento com Erro! Verificado por -
            <strong> {{ troca.responsavel.nome_funcional }}</strong>
          </h4>
          <b-form-textarea
            id="textarea"
            placeholder="Relate o erro..."
            v-model="troca.state_description"
            rows="4"
          ></b-form-textarea>
          <b-button
            class="mt-2"
          ...
          <b-button
            class="mt-2"
            variant="secondary"
            v-on:click="(troca.state = ''), (troca.state_description = '')"
            @click="() => Atualizando()"
          >
            Cancelar
          </b-button>
  ...
  <!-- Se eu já tenho um erro relatado, então essa minha div aparece com a informção do erro que eu já relatei -->
      <div v-if="troca.state === 'ERROR' && erro_existe === true" class="alerta">
        <b-alert show variant="danger">
          <h4 class="alert-heading">
            Documento com Erro! Verificado por -
            <strong> {{ troca.responsavel.nome_funcional }}</strong>
          </h4>
          <p>{{ troca.state_description }}</p>
          <b-button
            class="mt-2"
            variant="secondary"
      ...
      <div class="content" id="content-troca-servico">
        <hr />
        <div class="titulo">
          <h2>Processo Interno Para Troca de Serviço</h2>
        </div>
        <hr />
        <b-row>
          <b-col md="3">
        ...
        <b-row>
          <b-col md="6">
            <div class="test">
              <p>Placa/Nome:</p>
              <div class="campo">
                {{ troca.nome_solicitante }}
              </div>
            </div>
          </b-col>
          <b-col md="6">
            <div class="test">
              <p>Posto do Solicitante:</p>
              <div class="campo">
                {{ troca.posto_solicitante }}
              </div>
            </div>
          </b-col>
        </b-row>
        ...
        <hr />
        <div class="titulo">
          <h2>Dados do Servidor Solicitado</h2>
        </div>
        <hr />
        <b-row>
          <b-col md="6">
        ...
        <div class="titulo pt-4">
          <h2>Dados da Troca de Serviço</h2>
        </div>
        <hr>
        ...
        <b-row class="pt-4">
          <b-col md="6">
            <div class="test">
              <p>Dia a ser Trocado:</p>
              <div class="campo">
                {{ troca.dia_ser_trocadoUm }}
              </div>
            </div>
          </b-col>
          <b-col md="6">
        ...
        <b-row class="pt-4">
          <b-col md="6">
            <div class="test">
              <p>Dia Solicitado:</p>
              <div class="campo">
                {{ troca.dia_solicitadoUm }}
              </div>
            </div>
          </b-col>
          <b-col md="6">
            <div class="test">
              <p>Hora Serviço Solicitada:</p>
              <div class="campo">
                {{ troca.hr_serv_solicitadoUm }}
              </div>
            </div>
          </b-col>
        </b-row>
    ...
    <b-col>
      <b-button
        class="mr-3"
        variant="success"
        v-on:click="(troca.state = 'OK'), (troca.state_description = ''),
        (troca.id_administrativo = user.id)"
        @click="() => Atualizando()"
      >
      ...
      <b-button
        class="mr-3"
        variant="danger"
        v-on:click="troca.state = 'ERROR', (troca.id_administrativo = user.id)"
...
export default {
  ...
  methods: {
    CriaPDF() {
      var minhaTabela = document.getElementById("content-troca-servico")
        .innerHTML;
      var style = "<style>";
      style =
        style +
    ...
    loadTroca(){
      const url = `${baseApiUrl}/trocaservico/${this.$route.params.id}`;
    axios.get(url).then((res) => {
      this.troca = res.data;
      
      // console.log(res.data)
        const url = `${baseApiUrl}/assinatura/${res.data.id_assin_solicitante}`;
        axios.get(url).then((res) => {
          this.img = res.data;
          // console.log(res.data)
        });
    ...
    Atualizando() {
      const id = this.troca.id;

      axios
        .put(`${baseApiUrl}/trocaservico/${id}`, this.troca)
        .then(() => {
          this.loadTroca()
        })
        .catch((error) => {
